...
options:
	default_template	id of the template, which will be applied on 
				the startup
	window_width 		window width
	window_height 		window height
	charset 		html page encoding
	log 			path to the log file
	row_height		number of applications in a single column (so 
				why row?)
	background_color 	background color
	wallpaper 		wallpaper
	title 			title

section [Template] contains a template that determines which applications will 
be selected, you may have multiple such sections
options:
...
options:
	...
	conflicts 	list of the identifiers of applications (separated by 
			...
			this application
	checkedif 	condition (external program/script), which determines 
			if the application should be selected regardless of
			template
